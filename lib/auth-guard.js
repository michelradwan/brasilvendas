...
const SESSION_SECRET = process.env.SESSION_SECRET || process.env.JWT_SECRET || 'radwan-ads-master-secret-2026-institutional';
...
// Comparação de Senha com Timing-Safe Equal para evitar ataques de timing
function verifyPassword(providedPassword) {
    ...
    for (const validPass of validPasswords) {
        ...
        const bufB = Buffer.from(validPass.trim());
